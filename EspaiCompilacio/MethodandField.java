...
class method_info{
	...
	method_info(Long entCode, Long entException,Long exception_index0,Long access_flags0,Long name_index0,Long descriptor_index0,Long max_stack0,Long max_locals0,Vector<Long> code0) {
		...
		attributes_count=Long.valueOf(attributes.size());
	};
...
class Code_attribute extends attribute_info {
	...
	Code_attribute(Long ent, Long max_stack0,Long max_locals0,Vector<Long> code0) {
		...
		attribute_length=Long.valueOf(12+code0.size()); // 2(max_stack)+2(max_locals)+4(code_length)+2(exception_table_length)+2(attributes_count)+code.size
		max_stack=max_stack0;
		max_locals=max_locals0;
		code=code0;
		code_length=Long.valueOf(code.size());
	...
	void write(FileOutputStream f) {
		try {
			...
			attribute_length=Long.valueOf(12+code.size()); // 2(max_stack)+2(max_locals)+4(code_length)+2(exception_table_length)+2(attributes_count)+code.size
	...
	void show(){
		...
		Integer a;
		while (i.hasNext()){
			a= Integer.valueOf(i.next().toString());
			if (v==0l) {
				System.out.println();
				System.out.print("		 "+j+"	: ");
