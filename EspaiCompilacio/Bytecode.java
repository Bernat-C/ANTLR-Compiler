...
public void write() {
	...
	constant_pool_counter=Long.valueOf(constant_pool.size()+1);
...
private Long addClassName(String s) {
		...
		constant_pool.add(y);
		i=Long.valueOf(constant_pool.size());
...
private Long addMethodName(Long class_idx,String name,String desc) {
		...
		if (j==0L) {
			...
			constant_pool.add(d);
			j= Long.valueOf(constant_pool.size());
		}
		x=new CONSTANT_NameAndType_info(i,j);
		constant_pool.add(x);
		k= Long.valueOf(constant_pool.size());
		y=new CONSTANT_Methodref_info(class_idx,k);
		constant_pool.add(y);
		l= Long.valueOf(constant_pool.size());
...
public void addFunctionCode(Long ref,Long maxStack,Long nLocals,Vector<Long> code){
	...
	x=(CONSTANT_Methodref_info)constant_pool.elementAt( Integer.valueOf(ref.toString())-1);
	y=(CONSTANT_NameAndType_info)constant_pool.get( Integer.valueOf(x.name_and_type_index.toString())-1);
...
public void addMainCode(Long maxStack,Long nLocals,Vector<Long> code) {
	...
	x=(CONSTANT_Methodref_info)constant_pool.get(Integer.valueOf(mMain.toString())-1);
	y=(CONSTANT_NameAndType_info)constant_pool.get(Integer.valueOf(x.name_and_type_index.toString())-1);
	Long acc=ACC_PUBLIC|ACC_STATIC;
	z=new method_info(aCode,aExceptions,cException,acc,y.name_index,y.descriptor_index,maxStack,nLocals,code);
	methods.add(z);
...
public Long addConstant(String desc,String val){
	...
	case'I': {
		...
		CONSTANT_Integer_info x=new CONSTANT_Integer_info( Long.valueOf(i));
		constant_pool.add(x);
		ret= Long.valueOf(constant_pool.size());
	}
	break;
	case'Z': {
		Integer i;
	...
	case'S': {
		...
		constant_pool.add(x);
		Long i= Long.valueOf(constant_pool.size());
